...
window.saveHorariosConfig = () => {
    ...
    if (status) {
        status.textContent = 'âœ“ Horarios estándar guardados y aplicados correctamente.';
...
window.saveHorarioDailyOverride = async () => {
    ...
    try {
        ...
        if (status) {
            ...
            status.textContent = 'âœ“ Horario del día guardado correctamente para ' + fecha;
            setTimeout(() => { status.textContent = ''; }, 4000);
        }
            ...
            return `
                <div class="excel-hotel-section" data-hotel="${escapeHtml(hotel)}" style="background:white; border:1px solid #e2e8f0; border-radius:16px; overflow:hidden; margin-bottom:24px; box-shadow:0 4px 6px -1px rgba(0,0,0,0.05);">
                    ...
                    <div style="overflow:auto;"><table style="width:100%; border-collapse:collapse; min-width:980px;"><thead><tr style="background:#f8fafc;"><th style="padding:12px; border-bottom:1px solid #e2e8f0; text-align:left; font-size:0.7rem; color:#64748b; text-transform:uppercase;">Semana</th><th style="padding:12px; border-bottom:1px solid #e2e8f0; text-align:left; font-size:0.7rem; color:#64748b; text-transform:uppercase;">Empleado</th>${['LUN', 'MAR', 'MIE', 'JUE', 'VIE', 'SAB', 'DOM'].map(day => `<th style="padding:12px 8px; border-bottom:1px solid #e2e8f0; text-align:center; font-size:0.7rem; color:#64748b;">${day}</th>`).join('')}</tr></thead><tbody>${rows.map(row => `
                                    <tr class="excel-row-hover"><td style="padding:12px; border-bottom:1px solid #f1f5f9; white-space:nowrap; color:#64748b; font-size:0.8rem;">${window.fmtDateLegacy(row.weekStart)}</td><td style="padding:12px; border-bottom:1px solid #f1f5f9; min-width:220px; font-weight:600; color:#334155; font-size:0.85rem;">${String(row.empId).startsWith('vacante-') ? `<button type="button" class="btn-premium" data-vacancy="${escapeHtml(row.empId)}" data-hotel="${escapeHtml(row.hotel)}" data-week="${row.weekStart}" onclick="window.openWeeklyVacancy(this)" style="font-size:0.8rem;">${row.displayName} · Asignar sustituto</button>` : row.displayName}</td>${[0, 1, 2, 3, 4, 5, 6].map(offset => {
...
window.buildEmployeeProfileModel = (empleadoId, fechaReferencia) => {
    ...
    // INTENTO DE RECUPERAR TURNOS BASE DESDE ÃƒÃ…¡LTIMA CONSULTA GLOBAL (Aislamiento)
                ...
                <section class="emp-card glass" style="padding:20px; border-radius:24px; border:1px solid var(--border); background:rgba(37,99,235,0.05);">
                    <h4 style="margin:0 0 15px; font-size:0.8rem; font-weight:800; text-transform:uppercase; color:var(--accent);">Desglose del Saldo</h4>
                        <div style="display:flex; flex-direction:column; gap:12px;">
                            ...
                            <div style="display:flex; justify-content:space-between; font-size:0.85rem;"><span>Total planificado año:</span> <strong>${model.vacaciones.planificadas} d&iacute;as</strong></div>
    ...
    try {
        ...
        for (const hotel of hotelsToCheck) {
            ...
            const isSnapContentValid = (snap, wStart) => {
                ...
                if (!savedJson) return true; // fallback: sin json â†’ asumir válido (comportamiento legacy)
...
window.showPublishPreview = async () => {
    ...
    const publishNoteHtml = pendingResult.hasChanges
        ...
        : `<div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 16px; border-radius: 12px; font-size: 0.85rem; color: #475569;">
                    No se creará una nueva publicación porque la semana seleccionada ya coincide con el snapshot activo.
                </div>`;
...
window.buildPublicationSnapshotPreview = async (weekStart, hotelName = 'all') => {
    ...
    for (const snap of snapshots) {
        ...
        // Construir índice de aliases usando buildIndices para resolver "Sandra" â†’ UUID, etc.
        ...
        // Función helper: encontrar el empProfile siendo tolerante con nombres abreviados (S. Sánchez â‰ˆ Sandra Sánchez)
        ...
        (snap.rows || []).forEach(row => {
            ...
            if (empProfile) {
                ...
                // Todos los aliases del aliasMap que apunten a este empleado (ej. "Sandra" â†’ S. Sánchez)
                {
        ...
        // 2. Validación de Intercambios y Cambios de Turno
        events.forEach(ev => {
            // --- Helpers locales ---
            ...
            // REGLA PROTEGIDA: Intercambio activo con destino vacío â†’ BLOQUEO
            ...
            if (destinoEsVacio && !isPlaceholderDest) {
                if (_esIntercambio) {
                    ...
                    warnings.push('[AVISO] Cambio unilateral de ' + normEv.origen + ' el ' + evStart + ' sin compañero asignado.');
                }
                return;
            }
...
window.resolveEmployeeCanonicalId = (rawId, employees = window.empleadosGlobales || []) => {
    ...
    // Búsqueda adicional por primer nombre (ej. "Sandra" â†’ empleado con nombre "Sandra García" o "S. Sánchez")
...
window.employeeProfileActorLabel = (ev) => {
    ...
    const companero = ev?.payload?.companero || ev?.payload?.['compañero'] || ev?.payload?.['compañero'] || ev?.payload?.destinatario || '';
...
window.buildEmployeeProfileModel = (empId, refISO) => {
    ...
    // Fusionar y deduplicar eventos activos, globales y la caché anual
    const rawEvents = [
...
window.renderEmployeeProfile = () => {
    ...
    if (currentTab === 'summary') {
        ...
        const changeRegistryNote = changeSummary.eventTotal && changeSummary.eventTotal !== changeSummary.total
            ...
            : '';
        const changeSummaryHTML = `<div class="emp-leave-summary" style="margin-bottom:14px;">${changeChip('all', changeSummary.total, 'días con cambios')}${changeChip('requestedByEmployee', changeSummary.requestedByEmployee, `días solicitados por ${escapeHtml(emp.nombre || emp.id || 'el empleado')}`)}${changeChip('requestedByOthers', changeSummary.requestedByOthers, 'días solicitados por otros')}${changeRegistryNote}</div>`;
        tabContent = `<section class="emp-card glass emp-year-card" style="padding:20px; border-radius:18px; border:1px solid var(--border);"><div class="emp-year-header"><div><h3 style="margin:0 0 6px; font-size:0.9rem; font-weight:800;">Cambios de turno ${selectedYear}</h3><div class="emp-year-subtitle">Mostrando todo el año natural, del 01/01/${selectedYear} al 31/12/${selectedYear}.</div></div></div>${changeSummaryHTML}${renderRowsTable(changeRows, `No hay cambios de turno registrados en ${selectedYear}.`)}</section>`;
