...
window.saveChangeEdit = async (event) => {
    ...
    try {
        ...
        const companero = companeroInput || original.empleado_destino_id || original.payload?.companero || original.payload?.['compa\u00f1ero'] || original.payload?.destinatario || '';
...
window.renderEmployeeProfile = () => {
    ...
    const kpiHTML = `<div class="emp-kpi-grid" style="display:grid; grid-template-columns:repeat(${Math.max(4, kpiCards.length)}, minmax(100px, 1fr)); gap:8px; margin-bottom:10px;">${kpiCards.map(([label, value]) => {
        const isBaja = String(value || '').toUpperCase().includes('BAJA') && (label === 'Estado operativo' || label === 'Incidencia');
        return `<div class="emp-kpi-card glass" style="padding:14px; border-radius:16px; border:1px solid var(--border); background:${isBaja ? '#f1f5f9' : 'white'};"><label style="display:block; font-size:0.6rem; font-weight:800; color:var(--text-dim); text-transform:uppercase; margin-bottom:4px;">${escapeHtml(label)}</label><strong style="font-size:0.95rem; color:var(--text);">${escapeHtml(String(value ?? '-'))}</strong></div>`;
    }).join('')}</div>`;
    let tabContent = '';
    if (safeTab === 'summary') {
