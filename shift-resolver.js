...
tipo=${normalized.tipo}`);
    ...
    window.isValidShiftValue = (value) => {
        ...
        return ["M", "T", "N", "D", "P", "TP", "MANANA", "MAANA", "TARDE", "TURNOPARTIDO", "NOCHE", "DESCANSO"].includes(t);
    };
    ...
    window.normalizeShiftValue = (value) => {
        ...
        if (t === "P" || t === "TP" || t === "TURNOPARTIDO") return "TP";
    ...
    window.resolveEmployeeDay = ({ empleado = null, empleadoId, fecha, eventos = [], baseIndex = {}, hotel = '', skipChanges = false, _isResolvingRecursive = false, turnoBase = null }) => {
        ...
        // Propagar horario personalizado si está definido en eventos
        const activeEv = eventosActivos && eventosActivos.length ? eventosActivos[0] : null;
        result.horario = activeEv?.payload?.horario || activeEv?.horario || null;
