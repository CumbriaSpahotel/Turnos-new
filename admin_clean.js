...
window.editVacationByIndex = async (idx) => {
    ...
    await window.syncVacationFormHotel(); 
...
window.editBajaPeriod = async (id) => {
    try {
        const data = await window.TurnosDB.fetchEventos(); 
...
window.goToExcelRecord = (empId, date) => {
    ...
    window.excelFilters.search = empId === '??' ? '' : empId; 
    if (date) {
...
window.createPuestosPreviewModel = ({
    ...
    const operationalOccupantByOriginalEmployeeId = new Map(); 
    const originalEmployeeByOperationalOccupantId = new Map(); 
    ...
    try {
        ...
        hotelsList.forEach(hName => {
            ...
            dates.forEach(date => {
                ...
                if (weekExcelRows.length === 0) return; 
...
window.renderDashboard = async () => {
    ...
    try {
        ...
        window.publishPendingFromDashboard = window.publishPendingChangesForCard; 
