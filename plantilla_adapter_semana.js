...
(function () {
  ...
  const getLogo = (hotel) => LOGOS[hotel] || "";
    ...
    for (const emp of hotelGroup.orden_empleados) {
      for (const day of weekDays) {
        ...
        if (typeof raw === "object" && raw !== null) {
          const tipo = (raw.TipoInterpretado || "").toLowerCase();
          const isCT = tipo.includes("c/t") || tipo.includes("cambio");
          
          // MOSTRAR TURNO REAL: Si es un cambio, usamos el TurnoOriginal (ej: 'Mañana')
          const etiqueta = isCT ? (raw.TurnoOriginal || "Cambio") : (raw.TipoInterpretado || raw.TipoAusencia || "Ausencia");
          
          grid[emp][day] = etiqueta;
          meta[emp][day] = Object.assign({}, meta[emp][day], { 
            isAbsence: !isCT, 
            absenceLabel: etiqueta,
            isCT: isCT 
          });

          if (!isCT) absenceCount[emp] = (absenceCount[emp] || 0) + 1;
  ...
  function renderHotelWeek(container, hotelGroup, weekDays, filters) {
    ...
    const body = display.map(emp => {
      ...
      const badges = `<span class="emp-badges">
        <span class="b" title="Noches en el rango selecc.">🌙 ${nights}</span>
        <span class="b" title="Descansos en el rango selecc.">D ${rests}</span>
      </span>`;

      const tds = weekDays.map(day => {
        // Proceso de limpieza del nombre del turno
        let label = grid[emp]?.[day];
        if (typeof label === 'object' && label !== null) {
          label = label.TurnoOriginal || label.TipoInterpretado || 'Ausencia';
        }
        label = String(label || '').trim();

        // Si el texto es el genérico "Cambio de Turno", intentamos buscar dentro
        if (label.toLowerCase().includes('cambio de turno') || label.toLowerCase().includes('c/t')) {
          // Si hay una flecha o algo, intentamos limpiar, si no, lo dejamos en blanco para que no ensucie
          label = label.replace(/Cambio de Turno/gi, '').replace(/C\/T/gi, '').trim();
          if (!label) label = '🔄'; 
        }
        ...
        if (l.includes('mañana') || l === 'm') cls = 'turno-mañana';
        else if (l.includes('tarde') || l === 't') cls = 'turno-tarde';
        else if (l.includes('noche') || l === 'n') { cls = 'turno-noche'; icon = ' 🌙'; }
        else if (l.includes('descanso') || l === 'd') cls = 'turno-descanso';
        else if (l.includes('vacaciones') || l === 'v') { cls = 'turno-vacaciones'; icon = ' 🏖️'; }
        else if (l.includes('baja') || l === 'b') { cls = 'turno-baja'; icon = ' 🏥'; }
        else if (l.includes('permiso')) { cls = 'turno-permiso'; icon = ' 📝'; }

        if (label.includes('🔄')) icon += ' 🔄';
        const cleanLabel = label.replace(/🔄/g, '').trim() || '—';
        const isWeekend = [6,0].includes(fromISO(day).getDay());
        const weekendCls = isWeekend ? ' class="weekend"' : '';

        return `<td${weekendCls}><span class="turno-pill ${cls}">${cleanLabel}${icon}</span></td>`;
      }).join("");
  ...
  // ---------- Ready ----------
  const init = (overrideData) => {
    let data = overrideData || window.FULL_DATA || window.DATA || {};
    if (!overrideData) {
        try {
          const local = localStorage.getItem('turnosweb_admin_data');
          if (local) {
            data = JSON.parse(local);
            console.log('Utilizando datos administrados localmente (localStorage)');
          }
        } catch(e) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    ["hotelSelect","dateFrom","dateTo"].forEach(id => {
      ...
      if (!el) return;
      el.addEventListener("change", () => { const f=currentFilters(); refreshEmployeeOptions(window.__DATA__,f); renderContent(window.__DATA__,f); });
      el.addEventListener("input",  () => { const f=currentFilters(); refreshEmployeeOptions(window.__DATA__,f); renderContent(window.__DATA__,f); });
    });

    const empFilter = document.getElementById("employeeFilter");
    if (empFilter) empFilter.addEventListener("change", () => renderContent(window.__DATA__, currentFilters()));
  });
})();
