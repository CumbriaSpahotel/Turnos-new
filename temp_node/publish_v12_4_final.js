...
async function publishV124() {
    ...
    console.log("--- PUBLICACIÓN CONTROLADA MOTOR V12.4: POSITION INHERITANCE ---");
    ...
    for (const item of scope) {
        ...
        let baseSnap = snaps.find(s => s.publicado_por !== "IA_V12_1_FINAL" && s.publicado_por !== "IA_V12_2_ENRICHMENT" && s.publicado_por !== "IA_V12_3_SUBSTITUTION_FIX" && s.publicado_por !== "IA_V12_4_POSITION_INHERITANCE");
        let rows = [];
        if (baseSnap) {
        ...
        if (hasCT || hasBadOrder) { console.error(`[${hotel} ${weekStart}] ❌ ERROR VALIDACIÓN. CT: ${hasCT}, Order: ${hasBadOrder}`); continue; }
        ...
        const record = {
            ...
            estado: "activo",
            publicado_por: "IA_V12_4_POSITION_INHERITANCE",
            resumen: { source: "admin_preview_resolved_v12_4", rollback_target: snaps[0]?.id || null, inheritance_fix: true }
        };
