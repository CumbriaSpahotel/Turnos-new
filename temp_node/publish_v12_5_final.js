...
function buildCell(code, extra = {}) {
    ...
    // REGLA V12.5: Solo añadir 🔄 si es un CAMBIO real, no por SUSTITUCION.
...
async function publishV125() {
    ...
    console.log("--- PUBLICACIÓN MOTOR V12.5: NO-ICON SUBSTITUTION ---");
    ...
    for (const item of scope) {
            ...
            weekDates.forEach(f => {
                ...
                if (absEv) {
                    ...
                    if (absEv.empleado_destino_id) {
                        ...
                        newCells[f] = buildCell(t?.turno, { origen: "SUSTITUCION", icons: [] }); // NO 🔄
                    } else {
        ...
        const record = {
            ...
            version: Math.max(nextVersion, 132), // v132 is the winner
            estado: "activo",
            publicado_por: "IA_V12_5_NO_ICON_SUBST",
            resumen: { source: "admin_preview_resolved_v12_5", rollback_target: snaps[0]?.id || null, substitution_icon_fix: true }
        };
        ...
        else console.log(`[${hotel} ${weekStart}] ✅ PUBLICADO V12.5. ID: ${insData[0].id} Version: ${insData[0].version}`);
    }
}
publishV125();
