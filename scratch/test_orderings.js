...
// Load resolver, rules, excel loader
...
async function run() {
    ...
    // Split the two critical events of Cristina:
    // Event A (Natalio): ID=51b9070c-6a44-4f2f-abdc-677543bc2fa2
    // Event B (Isabel Hidalgo): ID=0275f0f3-e9ee-454e-8520-6f8e831da7d5
    ...
    // Order 1: Natalio event processed last (Natalio will be added)
    const listOrder1 = [...otherEvents, evIsabel, evNatalio];
    await testWithEventsOrder(listOrder1, "Natalio last (Natalio should be added)", lastSnapRows);
    
    // Order 2: Isabel Hidalgo event processed last (Natalio will be missing)
    const listOrder2 = [...otherEvents, evNatalio, evIsabel];
    await testWithEventsOrder(listOrder2, "Isabel Hidalgo last (Natalio missing)", lastSnapRows);
}
