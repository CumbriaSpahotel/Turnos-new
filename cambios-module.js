    ...
    const CambiosModule = {
        ...
        openNewChangeModal: async () => {
            try {
                ...
                hotelSelect.innerHTML = `<option value="">Sin hotel</option>${(hotels || []).map(h => `<option value="${window.escapeHtml(h)}" ${h === selectedHotel && selectedHotel !== 'all' ? 'selected' : ''}>${window.escapeHtml(h)}</option>`).join('')}`;
